...
export default class ShoppingcarController {
  ...
  addProduct = (idProduct) => {
    ...
    if (product.quantity === 1) {
      const textProduct = `<div class="product-name" id="product-name-${product.id}">${product.name}</div>
        <div class="product-quantity" id="product-quantity-${product.id}">${product.quantity}</div>
